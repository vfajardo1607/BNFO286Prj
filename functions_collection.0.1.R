...
get.module.scores <- function(seurat.obj, module.feats.dir, signal.thold, is.ensembl=FALSE, reports.path){
  cat('\n\n')
  cat('############    -----------   Module Scoring    -----------    ############\n')
  ...
  data.path <- paste0(reports.path, '/data')
...
vln.plot <- function(seurat.obj, feature, slot='data', groups.tag, na.rm=TRUE, groups.of.int=NULL, groups.label=NULL, filter.tags=NULL, groups.to.filter=NULL, keep=TRUE, feature.thold=NULL, vln.type='violin', color='color', size.thold=0, file.name=NULL, adjust.val=0.8, trim.val=FALSE, plot.limits=NULL, add.points=FALSE, this.color.scale=NULL){
  ...
  switch(
    EXPR=vln.type,
    'violin'={
      tmp.ggplot <- ggplot(data=tmp.data, aes(x=groups.tag, y=feature, fill=color)) + geom_violin(alpha=0.8, trim=trim.val, adjust=adjust.val) + geom_boxplot(width=0.05, alpha=0.7, outlier.shape=NA) + scale_y_continuous(expand=expansion(add=0), breaks=scales::pretty_breaks(n=3)) + labs(x=paste0('Groups (', groups.tag, ')'), y=feature)
      # Adjust color scale.
    ...
    'density'={
      ...
      labs(x=feature, fill=paste0('Groups (', groups.tag, ')'), y='Density')
      # Adjust color scale.
      if(!is.null(this.color.scale)){
...
preprocess.obj <- function(seurat.obj, set.lab, clusts.lab, pr.infer, get.metadata=TRUE){
  # @ Definition of signatures in seurat object.
  tmp.reports.path <- paste0(reports.path, '/module_signatures_', set.lab)
  if(!dir.exists(tmp.reports.path)) dir.create(tmp.reports.path)
  seurat.obj <- get.module.scores(seurat.obj=seurat.obj, module.feats.dir=module.sigs.path, signal.thold=0, is.ensembl=FALSE, reports.path=tmp.reports.path)
  ...
  meta.data <- as.data.table(cbind(cell=Cells(seurat.obj), seurat.obj@meta.data, seurat.obj@reductions$umap@cell.embeddings))
  # @ Further tags.
  # Cell barcode.
  meta.data$cell <- as.character(meta.data$cell)
  # Update clone size tag solely by taking into acccount the overlap of both the gene expression and TCR datasets.
  tmp.data <- meta.data[!is.na(clonotype.tag), .(new.clon.size=.N), by=clonotype.tag]
  tmp.data <- merge(x=meta.data, y=tmp.data, by='clonotype.tag', sort=FALSE, all=TRUE)
  #   Sanity checks before proceeding to update actual seurat object.
  tmp.check.1 <- all(tmp.data[, cell]==Cells(seurat.obj))
  tmp.check.2 <- tmp.data[is.na(clonotype.tag), all(is.na(new.clon.size))]
  tmp.check <- tmp.check.1 & tmp.check.2
  if(!tmp.check) stop('Unexpected error.\n')
  seurat.obj@meta.data[, 'clon.size.tag'] <- tmp.data[, new.clon.size]
  # Polyspecificity tag (for both seurat object and metadata datatable).
  if(pr.infer){
    meta.data <- include.ps.info(meta.data=meta.data)
    seurat.obj@meta.data[, 'pr.tag'] <- meta.data[, pr.tag]
  }
...
depict.clone.sharing <- function(
  meta.data, tag.of.int, # Input data.
...
# tag.of.int - Tag name (as defined in the seurat object's metadata) listing value of interest (to be taken as reference group).
# vals.to.depict - Carachter, lists the sets of values defined for the tag of interest for which an output file (GSEA plot) should be output.
...
do.fgsea <- function(metrics.src, tag.of.int, metric='signal.to.noise', output.path, vals.to.depict=NULL, files.preffix){
  # ---> Predefinitions.
...
process.aggr.json <- function(obj.of.int){
  # General definitions.
  ...
  json.data <- json.data[str_detect(string=sample.id, pattern='^CoVi|^BeMe')]
...
get.tcr.by.tag.table <- function(obj.of.int, vdj.gene.info, tag.of.int, phase.info=TRUE, cr.info=TRUE){
  # ---> Prepare data.
  ...
  meta.data[, tmp.tag:=get(tag.of.int)]
...
get.sep.conv.table <- function(table.obj, extra.tag=NULL){
  # @ General definitions.
  # Current object.
  ...
  if(!is.null(extra.tag)) meta.data[, extra.tag:=get(extra.tag)] else meta.data[, extra.tag:=NA]
  # Previous object.
  ...
  if(!is.null(extra.tag)) prev.meta.data[, extra.tag:=get(extra.tag)] else prev.meta.data[, extra.tag:=NA]
  # Fill missing data.
  if(table.obj != 'PT-6-ALL-SF' & table.obj != 'PT-24-ALL-SF'){
    meta.data[, blood.draw.phase.tag:='convalescent']
    prev.meta.data[, blood.draw.phase.tag:='convalescent']
  }
  if(table.obj == 'HLTY-6-ExpB-SFC' | table.obj == 'HLTY-6-ExpC-ALL'){
    meta.data[, hospitalization.tag:='Healthy']
    prev.meta.data[, hospitalization.tag:='Healthy']
  }
  ...
  tmp.data <- meta.data[
    ...
    by=.(
      donor=donor.id.tag,
      hospital=hospitalization.tag,
      cluster=cluster,
      ag=virus.tag,
      phase=blood.draw.phase.tag,
      extra.tag=as.character(extra.tag)
    )
  ]
  ...
  if(!is.null(extra.tag)){
    tmp.data <- merge(x=tmp.data, y=post.counts, by=c('donor', 'hospital', 'ag', 'phase', 'extra.tag'))
  }else{
    tmp.data[, extra.tag:=NULL]
    post.counts[, extra.tag:=NULL]
    tmp.data <- merge(x=tmp.data, y=pre.counts, by=c('donor', 'hospital', 'ag', 'phase'))
  }
  ...
  if(!is.null(extra.tag)){
    tmp.data <- merge(x=tmp.data, y=pre.counts, by=c('donor', 'hospital', 'ag', 'phase', 'extra.tag'))
  }else{
    pre.counts[, extra.tag:=NULL]
    tmp.data <- merge(x=tmp.data, y=pre.counts, by=c('donor', 'hospital', 'ag', 'phase'))
  }
  # @ Sanity checks
  ...
  # @ Final formatting.
  cols.order <- if(!is.null(extra.tag)) c('donor', 'hospital', 'phase', 'ag', 'extra.tag', 'pre.counts', 'post.counts', clusters) else c('donor', 'hospital', 'phase', 'ag', 'extra.tag', 'pre.counts', 'post.counts', clusters)
  names(cols.order) <- if(!is.null(extra.tag)) c('Donor', 'Hospitalization status', 'Sample collention timepoint', 'Virus peptide pool', 'Extra tag', 'Pre', 'Post', clusters) else c('Donor', 'Hospitalization status', 'Sample collention timepoint', 'Virus peptide pool', 'Pre', 'Post', clusters)
  tmp.data <- tmp.data[, ..cols.order]
  tmp.data[, phase:=ifelse(test=tmp.data[, phase]=='convalescent', yes='Convalescet COVID-19', no='Acute COVID-19')]
...
list.samples.info <- function(table.obj){
  ...
  if(!file.exists(vdj.table.file)) stop('Aggregation table file does not exist.\n')
  ...
  # Retrieve vdj path
  aggr.table[, vdj.path:=str_replace(string=vdj.table[, clonotypes], pattern='/clonotypes.csv', replacement='')]
